...
import com.github.intellij.gno.quickfix.GnoDeleteImportQuickFix;
...
public class GnoImportReference extends FileReference {
  public GnoImportReference(@NotNull FileReferenceSet fileReferenceSet, TextRange range, int index, String text) {
  ...
  protected Object createLookupItem(PsiElement candidate) {
    if (candidate instanceof PsiDirectory) {
      return GnoCompletionUtil.createDirectoryLookupElement((PsiDirectory)candidate);
    }
  ...
  public LocalQuickFix[] getQuickFixes() {
    if (GnoPackageUtil.isBuiltinPackage(resolve())) {
      return new LocalQuickFix[]{new GnoDeleteImportQuickFix()};
    }

    List<LocalQuickFix> result = ContainerUtil.newArrayList();
    FileReferenceSet fileReferenceSet = getFileReferenceSet();
    if (fileReferenceSet instanceof GnoImportReferenceSet && !((GnoImportReferenceSet)fileReferenceSet).isRelativeImport()
        && !fileReferenceSet.isAbsolutePathReference()) {
      result.add(new GnoGetPackageFix(fileReferenceSet.getPathString()));
    }
