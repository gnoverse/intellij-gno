 ...
 */
public class GnoBuildMatcher {
  private static final Pattern WHITESPACES = Pattern.compile("\\s+");

  @NotNull private final GnoTargetSystem myTarget;

  public GnoBuildMatcher(@NotNull GnoTargetSystem target) {
    myTarget = target;
  }
  ...
  private boolean matchFile(@NotNull PsiFile file, boolean checkBuildFlags) {
    ...
    if (((GnoFile)file).hasCPathImport() && myTarget.cgoEnabled != ThreeState.YES) return false;

    return match(file.getName(), ((GnoFile)file).getBuildFlags(), checkBuildFlags);
  }
  ...
  public boolean matchBuildFlag(@NotNull String name) {
    ...
    if (GnoConstants.KNOWN_COMPILERS.contains(name)) {
      return myTarget.compiler == null || name.equals(myTarget.compiler);
    }
    if (GnoConstants.KNOWN_VERSIONS.contains(name)) {
      return myTarget.goVersion == null || GnoSdkUtil.compareVersions(myTarget.goVersion, StringUtil.trimStart(name, "go")) >= 0;
    }
    if ("cgo".equals(name)) {
  ...
  public boolean matchFileName(@NotNull String fileName) {
    ...
    name = StringUtil.trimEnd(FileUtil.getNameWithoutExtension(name), GnoConstants.TEST_SUFFIX);

    List<String> parts = StringUtil.split(name, "_");
    int n = parts.size();

    if (n >= 2 && GnoConstants.KNOWN_OS.contains(parts.get(n - 2)) && GnoConstants.KNOWN_ARCH.contains(parts.get(n - 1))) {
    ...
    if (n >= 1) {
      if (GnoConstants.KNOWN_OS.contains(parts.get(n - 1))) {
        return matchOS(parts.get(n - 1));
      }

      if (GnoConstants.KNOWN_ARCH.contains(parts.get(n - 1))) {
  ...
  private boolean matchOS(@NotNull String name) {
    ...
    return GnoConstants.LINUX_OS.equals(name) && GnoConstants.ANDROID_OS.equals(myTarget.os);
  }
}