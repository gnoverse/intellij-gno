...
public class GnoPathScopeHelper {
  @NotNull
  ...
  public static GnoPathScopeHelper fromReferenceFile(@NotNull Project project,
                                                    @Nullable Module module,
                                                    @Nullable VirtualFile referenceFile) {
    VirtualFile sdkHome = GnoSdkUtil.getSdkSrcDir(project, module);
    String sdkVersion = GnoSdkService.getInstance(project).getSdkVersion(module);
    boolean supportsInternalPackages = GnoVendoringUtil.supportsInternalPackages(sdkVersion);
    boolean supportsSdkInternalPackages = GnoVendoringUtil.supportsSdkInternalPackages(sdkVersion);
    boolean vendoringEnabled = GnoVendoringUtil.isVendoringEnabled(module);
    Set<VirtualFile> sourceRoots = vendoringEnabled ? GnoSdkUtil.getVendoringAwareSourcesPathsToLookup(project, module, referenceFile)
                                                    : GnoSdkUtil.getSourcesPathsToLookup(project, module);
    return new GnoPathScopeHelper(sourceRoots, sdkHome, supportsInternalPackages, supportsSdkInternalPackages, vendoringEnabled);
  }

  private GnoPathScopeHelper(@NotNull Set<VirtualFile> roots,
  ...
  public boolean couldBeReferenced(@NotNull VirtualFile declarationFile, @Nullable VirtualFile referenceFile) {
    ...
    String importPath = GnoSdkUtil.getRelativePathToRoots(declarationDirectory, myRoots);
    if (importPath == null) {
      return false;
    }
    ...
    if (referenceDirectory != null) {
      if (myVendoringEnabled && GnoSdkUtil.isUnreachableVendoredPackage(declarationDirectory, referenceDirectory, myRoots)) {
        return false;
      }
      ...
      if (mySupportsInternalPackages || mySupportsSdkInternalPackages && declarationIsInSdk) {
        if (GnoSdkUtil.isUnreachableInternalPackage(declarationDirectory, referenceDirectory, myRoots)) {
          return false;
        }
      }
      if (declarationIsInSdk && GnoSdkUtil.findParentDirectory(declarationDirectory, myRoots, GnoConstants.TESTDATA_NAME) != null) {
        return false;
      }
      else {
    ...
    return GnoPsiImplUtil.allowed(declarationFile, referenceFile) && !isShadowedImportPath(declarationDirectory, importPath, myRoots);
  }
