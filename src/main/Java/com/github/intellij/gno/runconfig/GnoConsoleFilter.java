...
import com.github.intellij.gno.codeInsight.imports.GnoGetPackageFix;
import com.github.intellij.gno.sdk.GnoPackageUtil;
import com.github.intellij.gno.util.GnoPathResolveScope;
...
public class GnoConsoleFilter implements Filter {
    ...
    public GnoConsoleFilter(@NotNull Project project) {
        this(project, null, null);
    }

    public GnoConsoleFilter(@NotNull Project project, @Nullable Module module, @Nullable String workingDirectoryUrl) {
        myProject = project;
        myModule = module;
    ...
    public Result applyFilter(@NotNull String line, int entireLength) {
        ...
        if (goGetMatcher.find() && myModule != null) {
            ...
            HyperlinkInfo hyperlinkInfo = new GnoGetHyperlinkInfo(packageName, myModule);
        ...
        if (FileUtil.isAbsolutePlatformIndependent(fileName)) {
            ...
            if (virtualFile == null && myModule != null) {
                virtualFile = findInGnoPath(fileName);
                if (virtualFile == null && fileName.startsWith("src/")) {
                    virtualFile = findInGnoPath(StringUtil.trimStart(fileName, "src/"));
                }
            }
            if (virtualFile == null) {
    ...
    private VirtualFile findSingleFile(@NotNull String fileName) {
        if (PathUtil.isValidFileName(fileName)) {
            ...
            if (!files.isEmpty()) {
                GlobalSearchScope goPathScope = GnoPathResolveScope.create(myProject, myModule, null);
            ...
            if (!files.isEmpty()) {
                GlobalSearchScope smallerScope = GnoUtil.moduleScopeWithoutLibraries(myProject, myModule);
    ...
    @Nullable
    private VirtualFile findInGnoPath(@NotNull String fileName) {
        return GnoPackageUtil.findByImportPath(fileName, myProject, myModule);
    }

    public static class GnoGetHyperlinkInfo implements HyperlinkInfo {
        private final String myPackageName;
        private final Module myModule;

        public GnoGetHyperlinkInfo(@NotNull String packageName, @NotNull Module module) {
        ...
        public void navigate(Project project) {
            GnoGetPackageFix.applyFix(project, myModule, myPackageName, false);
        }
    }
}
