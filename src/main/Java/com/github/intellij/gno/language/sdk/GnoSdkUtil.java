...
import com.github.intellij.gno.language.GnoEnvironmentUtil;
import com.goide.appengine.YamlFilesModificationTracker;
import com.goide.project.GnoApplicationLibrariesService;
import com.goide.project.GnoLibrariesService;
...
public class GnoSdkUtil {
    ...
    public static Collection<VirtualFile> getGnoPathsRootsFromEnvironment() {
        return GnoEnvironmentGnoPathModificationTracker.getGnoEnvironmentGnoPathRoots();
    }
    ...
    public static String retrieveGnoVersion(@NotNull String sdkPath) {
        try {
            ...
            if (sdkRoot != null) {
                ...
                VirtualFile versionFile = sdkRoot.findFileByRelativePath("src/" + GnoConstants.GO_VERSION_NEW_FILE_PATH);
                if (versionFile == null) {
                    versionFile = sdkRoot.findFileByRelativePath("src/" + GnoConstants.GO_VERSION_FILE_PATH);
                }
                if (versionFile == null) {
                    versionFile = sdkRoot.findFileByRelativePath("src/pkg/" + GnoConstants.GO_VERSION_FILE_PATH);
                }
                if (versionFile != null) {
    ...
    public static String adjustSdkPath(@NotNull String path) {
        ...
        if (possibleGCloudSdk.exists() && possibleGCloudSdk.isDirectory()) {
            if (isAppEngine(possibleGCloudSdk.getAbsolutePath())) {
                return possibleGCloudSdk.getAbsolutePath() + GnoConstants.APP_ENGINE_GO_ROOT_DIRECTORY_PATH;
            }
        }

        return isAppEngine(path) ? path + GnoConstants.APP_ENGINE_GO_ROOT_DIRECTORY_PATH : path;
    }
...
import com.goide.GnoConstants;
import com.goide.GnoEnvironmentUtil;
import com.goide.appengine.YamlFilesModificationTracker;
import com.goide.project.GnoApplicationLibrariesService;
import com.goide.project.GnoLibrariesService;
import com.goide.psi.GnoFile;
import com.intellij.execution.configurations.PathEnvironmentVariableUtil;
import com.intellij.openapi.application.ApplicationManager;
import com.intellij.openapi.module.Module;
import com.intellij.openapi.module.ModuleManager;
