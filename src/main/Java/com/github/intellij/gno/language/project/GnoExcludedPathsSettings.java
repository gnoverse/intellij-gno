...
@State(
        ...
        storages = {
                ...
                @Storage(value = StoragePathMacros.PRODUCT_WORKSPACE_FILE + "/plugin.xml")
        }
)
public class GnoExcludedPathsSettings extends SimpleModificationTracker implements PersistentStateComponent<GnoExcludedPathsSettings> {
    ...
    public static GnoExcludedPathsSettings getInstance(Project project) {
        return project.getService(GnoExcludedPathsSettings.class);
    }
    ...
    @Override
    public void loadState(@NotNull GnoExcludedPathsSettings state) {
