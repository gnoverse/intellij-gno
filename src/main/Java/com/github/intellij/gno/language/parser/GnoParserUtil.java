...
import com.github.intellij.gno.language.GnoParserDefinition;
import com.github.intellij.gno.language.GnoTypes;
...
public class GnoParserUtil extends GeneratedParserUtilBase {
    private static final Key<Map<String, Integer>> MODES_KEY = Key.create("MODES_KEY");

    @NotNull
    private static Map<String, Integer> getParsingModes(@NotNull PsiBuilder builder_) {
        Map<String, Integer> flags = builder_.getUserData(MODES_KEY);
        if (flags == null) builder_.putUserData(MODES_KEY, flags = new HashMap<>());
        return flags;
    }
    ...
    public static boolean consumeBlock(PsiBuilder builder_, @SuppressWarnings("UnusedParameters") int level) {
        ...
        do {
            ...
            if (type == GnoTypes.TYPE_ && nextIdentifier(builder_)) { // don't count a.(type), only type <ident>
                m.rollbackTo();
                return false;
            }
            i += type == GnoTypes.LBRACE ? 1 : type == GnoTypes.RBRACE ? -1 : 0;
    ...
    private static boolean nextIdentifier(PsiBuilder builder_) {
        ...
        while ((e = builder_.rawLookup(++i)) == GnoParserDefinition.WS || e == GnoParserDefinition.NLS) {
        }
        return e == GnoTypes.IDENTIFIER;
    }
    ...
    public static boolean withOff(PsiBuilder builder_, int level_, Parser parser, String... modes) {
        Map<String, Integer> map = getParsingModes(builder_);
    ...
    private static boolean withImpl(PsiBuilder builder_, int level_, String mode, boolean onOff, Parser whenOn, Parser whenOff) {
        Map<String, Integer> map = getParsingModes(builder_);
        int prev = map.getOrDefault(mode, 0);
    ...
    public static boolean prevIsType(@NotNull PsiBuilder builder_, @SuppressWarnings("UnusedParameters") int level) {
        ...
        return type == GnoTypes.ARRAY_OR_SLICE_TYPE || type == GnoTypes.MAP_TYPE || type == GnoTypes.STRUCT_TYPE;
    }
    ...
    public static boolean keyOrValueExpression(@NotNull PsiBuilder builder_, int level) {
        ...
        boolean r = GnoParser.Expression(builder_, level + 1, -1);
        if (!r) r = GnoParser.LiteralValue(builder_, level + 1);
        IElementType type = r && builder_.getTokenType() == GnoTypes.COLON ? GnoTypes.KEY : GnoTypes.VALUE;
    ...
    public static boolean enterMode(@NotNull PsiBuilder builder_, @SuppressWarnings("UnusedParameters") int level, String mode) {
        Map<String, Integer> flags = getParsingModes(builder_);
    ...
    private static boolean exitMode(@NotNull PsiBuilder builder_, @SuppressWarnings("UnusedParameters") int level, String mode, boolean safe) {
        Map<String, Integer> flags = getParsingModes(builder_);
        int count = flags.getOrDefault(mode, 0);
