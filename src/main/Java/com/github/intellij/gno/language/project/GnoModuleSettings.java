package com.github.intellij.gno.language.project;
...
@State(name = GnoConstants.GNO_MODULE_SESTTINGS_SERVICE_NAME, storages = @Storage(file = StoragePathMacros.MODULE_FILE))
public class GnoModuleSettings implements PersistentStateComponent<GnoModuleSettings.GnoModuleSettingsState> {
    ...
    public static void showModulesConfigurable(@NotNull Project project) {
        ...
        if (!project.isDisposed()) {
            ShowSettingsUtil.getInstance().editConfigurable(project, new GnoConfigurableProvider.GnoProjectSettingsConfigurable(project));
        }
    }
