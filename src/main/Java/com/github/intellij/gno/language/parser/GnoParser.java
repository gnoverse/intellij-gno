package com.github.intellij.gno.language.parser;

import com.intellij.lang.PsiBuilder;
import com.intellij.lang.PsiBuilder.Marker;
import static com.github.intellij.gno.language.GnoTypes.*;
import static com.github.intellij.gno.language.parser.GnoParserUtil.*;
...
public class GnoParser implements PsiParser, LightPsiParser {
    ...
    public void parseLight(IElementType t, PsiBuilder b) {
        ...
        else if (t == GNO_STATEMENT) {
            r = GnoStatement(b, 0);
        }
        else if (t == GNOTO_STATEMENT) {
            r = GnotoStatement(b, 0);
        }
    ...
    public static final TokenSet[] EXTENDS_SETS_ = new TokenSet[] {
            ...
            create_token_set_(ASSIGNMENT_STATEMENT, BREAK_STATEMENT, CONTINUE_STATEMENT, DEFER_STATEMENT,
                    ...
                    GNOTO_STATEMENT, GNO_STATEMENT, IF_STATEMENT, INC_DEC_STATEMENT,
    ...
    // ExpressionArgList '...'? ','?
    private static void ArgumentList_1_0(PsiBuilder b, int l) {
        if (!recursion_guard_(b, l, "ArgumentList_1_0")) return;
        boolean r;
        Marker m = enter_section_(b);
    ...
    /* ********************************************************** */
    // !('!' | '!=' | '%' | '%=' | '&&' | '&' | '&=' | '&^' | '&^=' | '(' | ')' | '*' | '*=' | '+' | '++' | '+=' | ',' | '-' | '--' | '-=' | '...' | '/' | '/=' | ':' | ';' | '<' | '<-' | '<<' | '<<=' | '<=' | '<NL>' | '=' | '==' | '>' | '>=' | '>>' | '>>=' | '[' | ']' | '^' | '^=' | 'type' | '{' | '|' | '|=' | '||' | '}' | break | case | chan | char | const | continue | decimali | default | defer | else | fallthrough | float | floati | for | func | gno | gnoto | hex | identifier | if | int | interface | map | oct | return | select | string | raw_string | struct | switch | var)
    ...
    private static boolean ExpressionListRecover_0(PsiBuilder b, int l) {
        ...
        if (!r) r = consumeToken(b, FUNC);
        if (!r) r = consumeToken(b, GNO);
        if (!r) r = consumeToken(b, GNOTO);
    ...
    /* ********************************************************** */
    // gno Expression
    public static boolean GnoStatement(PsiBuilder b, int l) {
        if (!recursion_guard_(b, l, "GnoStatement")) return false;
        if (!nextTokenIs(b, GNO)) return false;
        boolean r, p;
        Marker m = enter_section_(b, l, _NONE_, GNO_STATEMENT, null);
        r = consumeToken(b, GNO);
        p = r; // pin = 1
        r = r && Expression(b, l + 1, -1);
        exit_section_(b, l, m, r, p, null);
        return r || p;
    ...
    /* ********************************************************** */
    // gnoto LabelRef
    public static boolean GnotoStatement(PsiBuilder b, int l) {
        if (!recursion_guard_(b, l, "GnotoStatement")) return false;
        if (!nextTokenIs(b, GNOTO)) return false;
        boolean r, p;
        Marker m = enter_section_(b, l, _NONE_, GNOTO_STATEMENT, null);
        r = consumeToken(b, GNOTO);
        p = r; // pin = 1
        r = r && LabelRef(b, l + 1);
        exit_section_(b, l, m, r, p, null);
        return r || p;
    ...
    public static boolean Statement(PsiBuilder b, int l) {
        ...
        if (!r) r = GnoStatement(b, l + 1);
        if (!r) r = ReturnStatement(b, l + 1);
        if (!r) r = BreakStatement(b, l + 1);
        if (!r) r = ContinueStatement(b, l + 1);
        if (!r) r = GnotoStatement(b, l + 1);
    ...
    /* ********************************************************** */
    // !('!' | '&' | '(' | '*' | '+' | '-' | ';' | '<-' | '^' | 'type' | '{' | '|' | '|=' | '||' | '}' | break | case | char | const | continue | decimali | default | defer | else | fallthrough | float | floati | for | func | gno | gnoto | hex | identifier | if | int | interface | map | oct | return | select | string | raw_string | struct | switch | var)
    ...
    public static boolean Expression_0(PsiBuilder b, int l, int g) {
        ...
        while (true) {
            ...
            if (g < 0 && consumeTokenSmart(b, COND_OR)) {
                ...
                r = true;
                exit_section_(b, l, m, BUILTIN_CALL_EXPR, true, true, null);
            }
            else if (g < 7 && ArgumentList(b, l + 1)) {
                r = true;
                exit_section_(b, l, m, CALL_EXPR, true, true, null);
            }
            else if (g < 7 && TypeAssertionExpr_0(b, l + 1)) {
                r = true;
                exit_section_(b, l, m, TYPE_ASSERTION_EXPR, true, true, null);
            }
            else if (g < 7 && SelectorExpr_0(b, l + 1)) {
                r = Expression(b, l, 7);
                exit_section_(b, l, m, SELECTOR_EXPR, true, true, null);
            }
            else if (g < 7 && IndexOrSliceExpr_0(b, l + 1)) {
                r = true;
                exit_section_(b, l, m, INDEX_OR_SLICE_EXPR, true, true, null);
            }
            else {
    ...
    final static Parser E_recover_parser_ = (b, l) -> E_recover(b, l + 1);
    final static Parser Element_parser_ = (b, l) -> Element(b, l + 1);
    final static Parser ExpressionListRecover_parser_ = (b, l) -> ExpressionListRecover(b, l + 1);
    final static Parser ExpressionOrTypeWithRecover_parser_ = (b, l) -> ExpressionOrTypeWithRecover(b, l + 1);
    final static Parser StatementRecover_parser_ = (b, l) -> StatementRecover(b, l + 1);
    final static Parser Statements_parser_ = (b, l) -> Statements(b, l + 1);
    final static Parser TopLevelDeclarationRecover_parser_ = (b, l) -> TopLevelDeclarationRecover(b, l + 1);
}